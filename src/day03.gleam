...
fn calculate_priority(line: String, item_priority: Map(String, Int)) -> Int {
  ...
  assert Ok(common_character) =
    intersection
    |> set.to_list()
    |> list.first()
  assert Ok(result) =
    item_priority
    |> map.get(common_character)

  result
}

fn calculate_priority_part_2(
  group: List(String),
  item_priority: Map(String, Int),
) -> Int {
  group
  ...
  |> list.first()
  |> result.unwrap("")
  |> function.flip(map.get)(item_priority)
  |> result.unwrap(0)
}
...
pub fn run() {
  ...
  io.println(
    "[Day 3][Part 2] Priority sum: " <> int.to_string(priority_sum_part_2),
  )
}
